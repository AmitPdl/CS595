...
class up(nn.Module):
    ...
    def forward(self, x1, x2, x3, Inp):
        ...
        x = torch.cat([Inp-x3, x1], dim=1)
        mask, mask_e, mask_d = self.mask_gene(x3, x1, Inp)     
        x = self.cv(x*mask) #mask = [mask_x2_e, mask_x1_d]
        mask_avg = torch.mean(self.pooling(mask),dim=1,keepdim=True)
        mask_avg[mask_avg ==0] = 1
        x = x*(1/mask_avg)
...
#         self.outR = outconv(64, 3)
#         self.down = DOWN()
#         self.lrm = LRM(64)
#         self.ref = nn.Conv2d(64, 3 , 3, stride=1, padding=1)
...
#         # print(input_R.shape)
#         r_final = self.ref(input_R) 
        
#         rr = self.lrm(y1,input_R) #y1 = transmission component


#         if not self.training:
#             return F.sigmoid(y), F.sigmoid(r_final)
...
#         return F.sigmoid(y), F.sigmoid(r_final), rr ,list_map, list_R, list_mask, list_mask_encoder, list_map_encoder


class GT_Model(nn.Module):
    def __init__(self, encoder_I, encoder_R,inpenc):
        super(GT_Model, self).__init__()
        
        self.Encoder_I = Encoder(encoder_I)
        self.Encoder_R = Encoder(encoder_R)
        self.Encoder_Inp = Encoder(inpenc)
    ...
    def forward(self, input_I, input_R, input_Inp, map_thr=None, map_encoder=None):
        I_x1, I_x2, I_x3, I_x4, I_x5 = self.Encoder_I(input_I)
        R_x1, R_x2, R_x3, R_x4, R_x5 = self.Encoder_R(input_R)
        Inp_x1, Inp_x2, Inp_x3, Inp_x4, Inp_x5 = self.Encoder_Inp(input_Inp)

             
        y4, mask4,mask4_e,mask4_d= self.upy1(I_x5, I_x4, R_x4, Inp_x4)
        y3, mask3,mask3_e,mask3_d = self.upy2(y4, I_x3, R_x3, Inp_x3) 
        y2, mask2,mask2_e,mask2_d = self.upy3(y3, I_x2, R_x2, Inp_x2)
        y1, mask1,mask1_e,mask1_d = self.upy4(y2, I_x1, R_x1, Inp_x1)
        y = self.outTc(y1)
