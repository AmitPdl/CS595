...
class SimpleGate(nn.Module):
    ...
    def forward(self, x):
        ...
        a = self.conv(torch.cat([x1, x2], dim=1))
...
class DualStreamGate(nn.Module):
    ...
    def __init__(self, channels):
        super(DualStreamGate, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(channels, channels//2, 3, 1, 1)
        )
    ...
    def forward(self, x, y):
        x1, x2 = x.chunk(2, dim=1)
        y1, y2 = y.chunk(2, dim=1)
        a = self.conv(torch.cat([x1, y2], dim=1))
        b = self.conv(torch.cat([y1, x2], dim=1))
...
class LRM(nn.Module):
    def __init__(self, in_channels=64, num_blocks=[2, 4]):
        super().__init__()
        self.device = 'cuda'
