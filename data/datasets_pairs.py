...
class my_dataset(Dataset):
    ...
    def train_transform(self, img, label, patch_size=224):
        """对图片和标签做一些数值处理"""
        ih, iw,_ = img.shape
...
def read_txt(txt_name = 'image_pairs.txt',sample_num=50000):
    path_in = []
    path_gt = []
    paths =[]
    ...
    for path in paths_random:
        ...
        path_in.append(temp1[1:])
        path_gt.append(temp2[1:])
    return path_in,path_gt

class my_dataset_wTxt(Dataset):
    def __init__(self, rootA, rootA_txt, crop_size=224, fix_sample_A=50000, regular_aug=False):
    ...
    def reset(self):
        """Resets the dataset by reloading data and shuffling indices."""
        # Reload data
        self.load_data()
        ...
        self.imgs_in_A = [self.imgs_in_A[i] for i in indices]
        self.imgs_gt_A = [self.imgs_gt_A[i] for i in indices]

    def __getitem__(self, index):
        data_IN_A, data_GT_A, img_name_A = self.read_imgs_pair(self.imgs_in_A[index], self.imgs_gt_A[index],
                                                               self.train_transform, self.crop_size)
        
        return data_IN_A, data_GT_A, data_IN_A-data_GT_A, False
...
class FusionDataset(BaseDataset):
    def __init__(self, datasets, fusion_ratios=None, target_size=15000):
        ...
        self.target_size = target_size  # Use target_size if provided, otherwise default to combined dataset size
    ...
    def __getitem__(self, index):
        # Adjust index to loop within target size
        index = index % self.target_size
        residual = 1
    ...
    def __len__(self):
        # Return target_size if specified, otherwise return the combined size
