...
class DSRTestDataset(BaseDataset):
    def __init__(self, datadir, fns=None, size=None, enable_transforms=False, unaligned_transforms=False,
        ...
        self.flag = flag
        self.if_align = if_align
        # self.original_fns = fns or os.listdir(join(datadir, 'blended'))  # Store original list for reset
        self.original_fns = fns or [
            fn for fn in os.listdir(join(datadir, 'blended'))
            if os.path.splitext(fn)[1].lower() in IMAGE_EXTENSIONS
        ]
        
        # Load file names with size limit if applicable
        self.load_data()

    def load_data(self):
        """Load or reload dataset file names, applying any size limit if provided."""
        self.fns = self.original_fns[:-1]
        if self.size is not None:
            self.fns = self.fns[:self.size]

    def reset(self):
        """Resets the dataset by shuffling the file names."""
        self.load_data()
        random.shuffle(self.fns)  # Shuffle filenames for a new order in each epoch
...
# class FusionDataset(BaseDataset):
#     def __init__(self, datasets, fusion_ratios=None, target_size=15000):
#         self.datasets = datasets
#         self.size = sum([len(dataset) for dataset in datasets])
#         self.fusion_ratios = fusion_ratios or [1. / len(datasets)] * len(datasets)
#         self.target_size = target_size or self.size  # Default to combined dataset size if target_size not provided

#         # Generate a pool of all possible indices based on fusion ratios
#         self.all_indices = []
#         for i, dataset in enumerate(datasets):
#             # Calculate number of samples for this dataset based on fusion ratio
#             dataset_size = int(self.target_size * self.fusion_ratios[i])
#             dataset_indices = [(i, idx) for idx in range(len(dataset))]
#             self.all_indices.extend(dataset_indices[:dataset_size])

#         # Initially shuffle indices
#         self.shuffle_indices()

#         print('[i] using a fusion dataset: %d %s imgs fused with ratio %s, target size %d' % (
#             self.size, [len(dataset) for dataset in datasets], self.fusion_ratios, self.target_size))

#     def shuffle_indices(self):
#         # Randomly select target_size indices from the pooled indices for each epoch
#         self.selected_indices = random.sample(self.all_indices, self.target_size)
...
#     def __getitem__(self, index):
#         # Access the shuffled subset of indices
#         dataset_idx, sample_idx = self.selected_indices[index]
...
class FusionDataset(BaseDataset):
    def __init__(self, datasets, fusion_ratios=None, target_size=15000):
        ...
        self.generate_indices()  # Initialize the indices
        print('[i] using a fusion dataset: %d %s imgs fused with ratio %s, target size %d' % (
            self.size, [len(dataset) for dataset in datasets], self.fusion_ratios, len(self.selected_indices)))

    def generate_indices(self):
        # Randomly sample new indices based on the fusion ratio
        all_indices = []
        for i, dataset in enumerate(self.datasets):
            dataset_size = int(self.target_size * self.fusion_ratios[i])
            dataset_indices = [(i, idx) for idx in range(len(dataset))]
            all_indices.extend(dataset_indices[:dataset_size])
        self.selected_indices = random.sample(all_indices, self.target_size)
        
    def reset(self):
        self.generate_indices()  # Regenerate indices
